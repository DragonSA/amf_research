...
\begin{equation*}
 ...
 \begin{cases}
  ...
  \X - (1 - \eta)\S + c_i q_{t^{c}_i }e^{t + \Dt - t^{c}_i} & \text{with probability } \po\\
 \end{cases}
\end{equation*}
where
...
\begin{tabular}{ll}
 ...
 $q_{t^{c}_i}$  & is an indicator variable that default happens after coupon payment\footnote{$\Prob(q_{t^{c}_i} = 1 | q \in [t, t + \Dt]) = \frac{e^{-\lambda(t^{c}_i - t)} - e^{-\lambda\Dt}}{1 - e^{-\lambda\Dt}}$, the probability that the coupon arrives before default occurs, given that default occurs within the interval}
\end{tabular}
\end{savenotes}
...
\begin{align}
 V_t    &= e^{-r\Dt}(\Vu\pu + \Vd\pd + \X\po) + c_i e^{-(r + \lambda)(t^{c}_i - t)} \n
        &= e^{-r\Dt}\E[V_{t + \Dt}] + c_i e^{-(r + \lambda)(t^{c}_i - t)}
\end{align}
