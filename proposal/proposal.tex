...
\citet{I77} started the literature of pricing a convertible bond with extensions from \cite{BS77} and \cite{BS80}.  The original approach was to treat the bond and equity as components of the issuer's value and to treat default as when the issuer's value drops below a point where it can no longer meet it's financial obligations.  An overview of this type of approach is provided by \cite{N96} and criticisms are addressed by \cite{JT95}.  The main problems with this model is that the issuer's value is not directly observable, difficult to parameterise and all senior debt to the convertible bond also needs to be priced.
...
Also some Monte Carlo based pricing methods have been considered as proposed by \cite{B89} with \cite{G03} providing an optimisation approach to handle optimal early exercise of the American options.  Further improvements to the pricing methods were done by \cite{AKW08}.

Other methods used for pricing include a finite element method by \cite{BBH03} and a binomial tree method by \cite{TKN01} and \cite{AFV02}.
